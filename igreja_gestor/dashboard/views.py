...
def dashboard(request):
    ...
    movimentacoes = Movimentacao.objects.filter(data__month=timezone.now().month, data__year=timezone.now().year)

    # Contando aniversariantes do mês
    aniversariantes = Membro.objects.filter(data_nascimento__month=timezone.now().month)

    # Resumo financeiro
    total_entradas = movimentacoes.filter(tipo='entrada').aggregate(total=Count('id'))['total']
    total_saidas = movimentacoes.filter(tipo='saida').aggregate(total=Count('id'))['total']
